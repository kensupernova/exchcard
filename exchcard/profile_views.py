...
from exchcard.serializers import UserSerializer, CardSerializer, RegisterUserSerializer2

from exchcard.serializers import GetProfileSerializer, CreateProfileSerializer
from exchcard.serializers import GetProfileWithCardSerializer
from exchcard.serializers import AddressSerializer

from exchcard.serializers import RegisterUserAddressProfileSerializer
from exchcard.serializers import GetUserAddressProfileSerializer


from exchcard.utils import generateToken
from exchcard.utils import count_arrive_travel
...
## admin user or staff user can create exchcard with user_id and profile_id
@api_view(["POST"])
...
def api_update_profile(request, format=None):
    ...
    if request.method == "PUT":
        ...
        print "------------------------------"
        print "update exchcard"
        # serializer = AddProfileSerializerWithIds(exchcard, data=request.data)
...
def api_get_random_profile(request, format=None):
    """
    get one exchcard for sending card, randomly, later, use algorithm to match
    :param Request:
    :return:
    """
    if request.method == "GET":
        ...
        ### search the sender exchcard id at the same time
