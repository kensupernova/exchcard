...
from exchcard.models import Address, Card, DianZan, CardPhoto

from exchcard.permissions import IsOwnerOrReadOnly, IsProfileUserOrStaffUser, IsProfileUser
from exchcard.permissions import IsSenderOrReadOnly, IsSenderStaffOrReadOnly

from exchcard.serializers import CreateCardSerializer, CardSerializer
from exchcard.serializers import DianZanSerializer

import time
import utils

exchcard_api_endpoint = "http://exchcard.applinzi.com/exchcard/"
...
def api_root(request, format=None):
    return Response({
        ...
        "exchcard-user-address-register": reverse("exchcard-user-address-register", request=request, format=format),
        "exchcard-user-address-register2": reverse("exchcard-user-address-register2", request=request, format=format),
...
class CardDetail(generics.RetrieveAPIView):
    ...
    #     serializer.save(fromsender = self.request.user.exchcard)
    #
...
def api_add_new_card(request):
    if request.method == "POST":
        ## the sender_id must be the current log in user exchcard
...
def api_receive_a_card(request):
    ...
    if request.method == "POST":
        ...
        profile_from_request_user = Profile.objects.get(profileuser
                                                   =request.user)
        ## check whether the exchcard id in the request data is the the same
        ...
        if profile_from_request_user.id == recipient_profile_id:
            ...
            return Response({"details": "exchcard id!= card recipient exchcard id, card name ",
...
def api_receive_a_card_with_photo(request):
    ...
    if request.method == "POST":
        ...
        profile_from_request_user = Profile.objects.get(profileuser
                                                   =request.user)
        ## check whether the exchcard id in the request response_data is the the same
        ## with the one of the request
